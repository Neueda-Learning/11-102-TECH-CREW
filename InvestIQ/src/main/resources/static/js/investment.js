...
function initializeBuySymbolSearch() {
  ...
  buyRefs.symbolInput.addEventListener("blur", function () {
   ...
   buyRefs.suggestions.addEventListener("mousedown", async function (event) {
     ...
     // Auto-fetch current price from API
     await fetchAndPopulateCurrentPrice(symbol);
   });
...
async function fetchAndPopulateCurrentPrice(symbol) {
   ...
   try {
     ...
     if (quote) {
       const price = parsePriceFromQuote(quote);
       if (Number.isFinite(price) && price > 0) {
         currentPriceInput.value = price.toFixed(2);
       }
     }
   } catch (error) {
     console.warn("[Investment] Failed to fetch current price for", symbol, error);
   }
}
